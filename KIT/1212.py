...
for i, case in enumerate(cases):
    ...
    for j, points in enumerate(data_points):
        ...
        # 自定义颜色映射，将误差值映射为浅色调
        colors = [(1, 1, 1), (0.678, 0.847, 0.902), (1, 0.627, 0.478)]  # 白色到浅蓝色到浅红色
        cmap = LinearSegmentedColormap.from_list('custom_cmap', colors)

        # 设置规范，使颜色范围从0到最大误差值
        norm = Normalize(vmin=0, vmax=np.max(absolute_error))

        # 绘制绝对误差热场图
        ax_diff = axes[j, i * 2 + 1]
        divider2 = make_axes_locatable(ax_diff)
        cax2 = divider2.append_axes("right", size="5%", pad=0.2)  # 调整pad值以增加间距
        c2 = ax_diff.tricontourf(actual_data['x'], actual_data['y'], absolute_error, levels=100, cmap=cmap, norm=norm)
...
# 确保保存路径存在
save_path_svg = 'D:\\桌面\\科研绘图1.svg'
save_path_png = 'D:\\桌面\\科研绘图1.png'
