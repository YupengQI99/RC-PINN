...
# 调整子图间距
fig1.subplots_adjust(hspace=0.3, wspace=0.3)  # 减少子图之间的距离
fig2.subplots_adjust(hspace=0.3, wspace=0.3)  # 减少子图之间的距离
...
colors = [(1, 1, 1), (0.8, 0.8, 1), (0.6, 0.6, 1), (0.4, 0.4, 1), (0.2, 0.2, 1), (0, 0, 1)]  # 从白色到淡蓝色
...
for i, case in enumerate(cases):
    ...
    for j, points in enumerate(data_points):
        ...
        cbar1.ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{x:.1f}'))  # 格式化标签
        ax_temp1.set_title(f'{case} {points}', fontsize=22)  # 增大标题字体
        ax_temp1.set_xticks([])  # 移除x轴刻度
        ax_temp1.set_yticks([])  # 移除y轴刻度
        ax_temp1.set_aspect('equal')  # 确保热场图是正方形

        # 绘制第一个程序的绝对误差热场图
        ax_diff1 = axes1[j, i * 2 + 1]
        divider2 = make_axes_locatable(ax_diff1)
        cax2 = divider2.append_axes("right", size="5%", pad=0.2)  # 调整pad值以增加间距
        c2 = ax_diff1.tricontourf(actual_data['x'], actual_data['y'], absolute_error1, levels=100, cmap=cmap, norm=norm)
        cbar2 = fig1.colorbar(c2, cax=cax2)
        cbar2.ax.set_ylabel('')  # 移除颜色条标签
        cbar2.set_ticks(cbar2.get_ticks()[::2])  # 只显示每隔一个刻度的标签
        cbar2.ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{x:.1f}'))  # 格式化标签
        ax_diff1.set_title(f'{case} Abs Error {points}', fontsize=22)  # 增大标题字体
        ax_diff1.set_xticks([])  # 移除x轴刻度
        ax_diff1.set_yticks([])  # 移除y轴刻度
        ax_diff1.set_aspect('equal')  # 确保热场图是正方形

        # 绘制第二个程序的预测温度图
        ax_temp2 = axes2[j, i * 2]
        divider3 = make_axes_locatable(ax_temp2)
        cax3 = divider3.append_axes("right", size="5%", pad=0.2)  # 调整pad值以增加间距
        c3 = ax_temp2.contourf(x_grid, y_grid, T_pred2, levels=100, cmap='jet')
        cbar3 = fig2.colorbar(c3, cax=cax3)
        cbar3.ax.set_ylabel('')  # 移除颜色条标签
        cbar3.set_ticks(cbar3.get_ticks()[::2])  # 只显示每隔一个刻度的标签
        cbar3.ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{x:.1f}'))  # 格式化标签
        ax_temp2.set_title(f'{case} {points}', fontsize=22)  # 增大标题字体
        ax_temp2.set_xticks([])  # 移除x轴刻度
        ax_temp2.set_yticks([])  # 移除y轴刻度
        ax_temp2.set_aspect('equal')  # 确保热场图是正方形

        # 绘制第二个程序的绝对误差热场图
        ax_diff2 = axes2[j, i * 2 + 1]
        divider4 = make_axes_locatable(ax_diff2)
        cax4 = divider4.append_axes("right", size="5%", pad=0.2)  # 调整pad值以增加间距
        ...
        cbar4.ax.set_ylabel('')  # 移除颜色条标签
        cbar4.set_ticks(cbar4.get_ticks()[::2])  # 只显示每隔一个刻度的标签
        cbar4.ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{x:.1f}'))  # 格式化标签
        ax_diff2.set_title(f'{case} Abs Error {points}', fontsize=22)  # 增大标题字体
        ax_diff2.set_xticks([])  # 移除x轴刻度
        ax_diff2.set_yticks([])  # 移除y轴刻度
        ax_diff2.set_aspect('equal')  # 确保热场图是正方形

# 确保保存路径存在
save_path_svg1 = 'D:\\桌面\\科研绘图pinn1101.svg'
save_path_png1 = 'D:\\桌面\\科研绘图pinn1101.png'
save_path_svg2 = 'D:\\桌面\\科研绘图nn1101.svg'
save_path_png2 = 'D:\\桌面\\科研绘图nn1101.png'
...
fig1.savefig(save_path_png1, format='png', dpi=300)  # 保存为PNG格式，设置dpi为300以确保清晰度
fig2.savefig(save_path_svg2, format='svg')
fig2.savefig(save_path_png2, format='png', dpi=300)  # 保存为PNG格式，设置dpi为300以确保清晰度
plt.show()
